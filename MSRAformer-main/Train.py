...
from utils.distributed_utils import MetricLogger, SmoothedValue
    ...
    for i, pack in enumerate(train_loader, start=1):
        ...
        if i % 20 == 0 or i == total_step:
            print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], '
                  ' lateral-5: {:0.4f}], lateral-4: {:0.4f}],lateral-3: {:0.4f}], lateral-2: {:0.4f}], lateral-1: {:0.4f}]'.
                  format(datetime.now(), epoch, opt.epoch, i, total_step,
                          loss_record5.show(),loss_record4.show(),loss_record3.show(),loss_record2.show(),loss_record1.show()))

    save_path = 'snapshots/{}/'.format(opt.train_save)
    os.makedirs(save_path, exist_ok=True)

    if (epoch + 1) % 1 == 0:
        meandice = tst(model, test_path)
        ...
        if meandice > float(best):
            ...
            best = fp.read()
            fp.close()
            torch.save(model.state_dict(), save_path + 'MSRAformer-best.pth')
            print('[Saving Snapshot:]', save_path + 'MSRAformer-best.pth', meandice, '[best:]', best)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', type=str,
                        default='./dataset/CVC-ClinicDB/train', help='path to train dataset')

    parser.add_argument('--test_path', type=str,
                        default='./dataset/CVC-ClinicDB/test', help='path to testing Kvasir dataset')

    parser.add_argument('--train_save', type=str,
                        default='MSRAformer-best')

    opt = parser.parse_args(args=[])

    # ---- build models ----
    torch.cuda.set_device(0)
    model = msraformer().cuda()
